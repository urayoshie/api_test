// Rails API 側の ベースURL
const baseURL = 'https://secret-meadow-31738.herokuapp.com';
...
// 投稿ボタンのDOM 要素オブジェクトを取得
const postButton = document.getElementById('post-button');
// タイトル入力フィールドの DOM 要素オブジェクトを取得
const titleElement = document.getElementById('post-title');
// 内容入力エリアのDOM 要素オブジェクトを取得
const contentElement = document.getElementById('post-content');

// 入力内容をAPI側のサーバーに送信して保存する関数
const registerPost = async () => {
  // ローカルの Rails サーバーのpost#create に対応するURL
  const url = `${baseURL}/posts`;
  // 入力内容をAPI側から受け取れるパラメータ形式に加工
  const postParams = {
    post: {
